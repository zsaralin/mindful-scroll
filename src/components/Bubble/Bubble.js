...
export function setIsHiding(input) {isHiding = input}
...
export async function hideColourPreview(x,y) {
    if (isHiding || getIsChanging()) return
        ...
        gsap.to("#bubble", {opacity: 0, duration: 1, onComplete() {teleportFeedback(x,y)}})
...
export const moveFeedback = async (prevX, prevY, x, y) => {
    if (isMoving) {
        ...
        gsap.to("#bubble", {duration: Math.round(dist/10), x: loc[0] +'px', y: loc[1] + 'px', ease: "power1.inOut"})
        await delay(6000);
        isMoving = false;
    }
}
